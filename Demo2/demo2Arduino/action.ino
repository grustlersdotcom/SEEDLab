...
const float voltage_max = 3.0;//4.00
const float min_effective_voltage = 1.7;
const float min_effective_turning_voltage = 2.5;//2.5
const float straightness_correction_factor = 0.99; // limit ahead wheel's voltage to this factor of other wheel target voltage
const float allowed_delta_threshold = 1.00; // multiplied by circumference to get threshold distance delta
...
void apply_simultaneous_voltage(float left_target_voltage, float right_target_voltage) {
  float speed_factor_L = 1.0; ////straight 0.44
  float speed_factor_R = 1.0;  // 0.479
// 3ft L-0.459 R- 0.479 
// 5ft L-0.459 R-0.479
// 7ft L-0.468 R 0.479
//angles L-0.44 R-0.479
//180 L-0.445 R-0.479
  // Create local variables to modify
  float scaled_left_voltage = left_target_voltage * speed_factor_L;
  float scaled_right_voltage = right_target_voltage * speed_factor_R;

  // Apply the scaled voltage
  motorShield.setM2Speed(scaled_left_voltage * (255.0 / voltage_max));
  motorShield.setM1Speed(scaled_right_voltage * (255.0 / voltage_max));

  // Update statistics with the scaled voltages
  update_statistics(scaled_left_voltage, scaled_right_voltage);
}
...
void move_straight() {
  ...
  float straigtness_error = left_current_position - right_current_position;
  float correction = straightness_pid(straigtness_error);
  float left_adj_v = (left_target_voltage - correction) * 0.7;
  float right_adj_v = (right_target_voltage + correction) * 0.7;

  // apply
  apply_simultaneous_voltage(left_adj_v, right_adj_v);
}

void move_angle() {
  float left_target_voltage = get_target_voltage(0) * 0.44;
  float right_target_voltage = get_target_voltage(1) * 0.479;
...
bool has_reached_target() {
  ...
  if (move_mode) {
    ...
    threshold = 0.012; // 0.1 in per wheel tolerance while turning
    //0.001 when going straight
  }
