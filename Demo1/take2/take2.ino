...
//Robot constants
const float wheelRadius = 0.073;  //Wheel radius in meters
...
const int CPR = 3200;  //Encoder counts per revolution
...
void loop() {
  ...
  if (millis() - lastTime >= desiredLoopTime) {
    ...
    if (turnAngle != 0) {
      Serial.println("Main loop angle, (shouldn't print)");
...
void moveStraight(float distanceInInches) {
  ...
  while (true) {
    ...
    //Serial.println("while true loop (once)");
    //Stopping condition based on tolerance
    if (fabs(distanceInMeters - leftWheelDistance) < tolerance && fabs(distanceInMeters - rightWheelDistance) < tolerance) {
  ...
  while (true) {
    ...
    if (fabs(currentAngle - radians) < tolerance) {
      ...
      if (degrees > 0) {
        ...
        motorShield.setM1Speed(Voltage[0]);   //Left motor forward
        motorShield.setM2Speed(Voltage[1]);  //Right motor reverse
      }
...
void updateWheelMetrics() {
  static long leftCountPrev = 0;
  static long rightCountPrev = 0;
  ...
  float timeDelta = (currentTime - lastTime) / 1000.0;  //Convert microseconds to seconds
  ...
  actual_speed[0] = (leftWheelDistance - leftCountPrev * distancePerTick) / timeDelta;  //Left wheel speed
  actual_speed[1] = (rightWheelDistance - leftCountPrev * distancePerTick) / timeDelta;  //Right wheel speed
...
void computePID(int index, float desired_pos[], float actual_pos[], float deltaTime) {
  ...
  Voltage[index] = constrain(Kp_pos * (desired_speed[index] - actual_speed[index]),-maxVoltage,maxVoltage);
  static uint64_t countUp2 = 0;
  if (countUp2 == 10000){
    Serial.print("AS: ");
    Serial.print(actual_speed[0]);
    Serial.print(" V: ");
    Serial.print(Voltage[0]);
    Serial.print(" DS: ");
    Serial.println(desired_speed[0]);
